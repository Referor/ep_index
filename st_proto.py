...
df_cov_sc=pd.read_excel('data/scaled_data.xlsx', index_col='month')
fig = px.line(df_cov_sc, x='months', y='value', color='query', hover_name="query").for_each_trace(lambda t: t.update(name=t.name.split("=")[1]))
fig.update_traces(mode='markers+lines')
fig.update_layout(legend_traceorder="reversed")
st.sidebar.title('Navigation')
selection = st.sidebar.radio("Go to", ['О проекте','Визуализация исходных данных', 'Поиск взаимосвязей'])
...
if selection == 'Поиск взаимосвязей':
	...
	df_cov_true=pd.read_excel('data/corr_only_true_data.xlsx')
